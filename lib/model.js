  ...
  function Model(attrs) {
    ...
    for (var attr in attrs) {
      if (Model.attrs[attr]) {
        ...
        if (type(Model.attrs[attr].type) === 'model'){
          var subModel = Model.attrs[attr].type;
          this.attrs[attr] = new subModel(clone(attrs[attr]));
        }
        else {
          this.attrs[attr] = clone(attrs[attr]);
        }
      }
    }

    Model.emit('initialize', this, attrs);
  }
